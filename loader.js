...
function validateGameParameters(config, gameKey, difficulty, shottype, route) {
  ...
  if (config.route) {
    if (route && !CONSTANTS.VALID_ROUTES.includes(route)) {
      redirectWithError(errorUrl);
      return false;
    }
  }
